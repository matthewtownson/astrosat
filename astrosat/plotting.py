...
class Plot:
        ...
        for i_d in range(len(self.stars.bright_stars)):
            ...
            if abs(RAanglediff) < self.AS.parameters.radius*1.5:
                if abs(DECanglediff) < self.AS.parameters.radius*1.5:
                    ...
                    plt.plot(RAstar*12./180., DECstar, color='k', marker='o', markersize=(r*-1+1)*self.AS.markersize)
        ...
        for i_sat in range(len(sat_dict)):
            ...
            if mag_sat is not None:
                ...
                if len(sat_dict[i_sat][4])==1:
                    plt.plot(sat_dict[i_sat][4], sat_dict[i_sat][5], 'x', color='r',markersize=(r*-1+1) * self.AS.markersize)
                else:
                    ...
                    DECextrap = f(RAtemp)
                    plt.plot(RAtemp*12./180., DECextrap, linestyle='--', color='r', linewidth=(r*-1+1)* self.AS.markersize)
    ...
    def plot_legend(self, labels):
        ...
        for label in labels:
            ...
            r = abs(-1*(Mmin-label)/float(Mmax-Mmin))
            r = numpy.clip(r, 0, 1.)
            p2, = plt.plot([],[], color='r', linestyle='--', linewidth=(r*-1+1)*self.AS.markersize, label='mag %i' % label)
            label_list.append(p2)
        for label in labels:
            # r = 1+1*(Mmin-label)/float(Mmax-Mmin)
            r = abs(-1*(Mmin-label)/float(Mmax-Mmin))
            r = numpy.clip(r, 0, 1.)
            p2, = plt.plot([],[], 'x',color='r', markersize=(r*-1+1)*self.AS.markersize, label='mag %i' % label)
            label_list.append(p2)
